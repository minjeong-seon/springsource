...
//After(메소드 호출 후 예외 발생여부와 관계 없이), Around(메소드 호출 이전과 이후 모두 적용)

@Component("log")
@Aspect
public class LogAdvice {
	// 표현식 : (value="execution(리턴타입 패키지명.클래스명.메소드명)")
	// =>특정 메소드만 advice를 함
	// * : 리턴타입
	// com.spring.aop : 패키지명
	// Product : 클래스명
...
//	System.out.println("[공통로그] 비즈니스 로직 예외 발생 후 호출");
//	}
	
//	@AfterReturning(value="execution(* com.spring.aop.Product.getInfo())")
//	public void afterThrowingLog() {
//	System.out.println("[공통로그] 비즈니스 로직 수행 후 예외 없을 때만 호출됨");
//	}
	
	@Around(value="execution(* com.spring.aop.Product.getInfo())")
	public void aroundLog(ProceedingJoinPoint pjp) {
	System.out.println("[공통로그] 비즈니스 로직 전후 호출");
	try {
		pjp.proceed();	// <== 처리해야 하는 메소드 호출하려면 필요한 코드
	} catch (Throwable e) {
		e.printStackTrace();
	}
	System.out.println("[공통로그] 비즈니스 로직 전후 호출(예외 있거나 말거나)");
	}
