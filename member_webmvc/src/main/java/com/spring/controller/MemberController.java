...
public class MemberController {
	...
	@PostMapping("/dupId")
	@ResponseBody // 컨트롤러 작업이 완료될 때 결과값으로 리턴시킴(=viewResolver 동작X)
	public String dupIdCheck(String userid) {
		log.info("중복 아이디 검사.." + userid);

		boolean idCheck = service.dupId(userid);
		if (idCheck) {
			return "true"; // /WEN-INF/views/true.jsp
		} else {
			return "false"; // /WEN-INF/views/false.jsp
		}
	}
	...
	public String changePwdPost(ChangeDTO changeDTO, HttpSession session) {
		...
		if(changeDTO.passwordEquals()) {
			if(service.changePwd(changeDTO)) {
				//true : 비번 변경 ==> session 전달+/login페이지
				session.invalidate();
				return "redirect:/member/login";
			}
		}
