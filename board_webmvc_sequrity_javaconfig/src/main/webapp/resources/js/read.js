...
// 로그인 정보와 작성자 정보가 다르면 '수정'버튼이 보이지 않음 => 수정 버튼 있을 때만 목록버튼 활성화
//(스크립트 코드는 위에서 아래 순서로 실행되므로, 에러가 나는 부분부터 해결해야 함)
// /board/modify 전송
...
chat.addEventListener("click", (e) => {
  ...
  //로그인 사용자 정보 가져와서 변수 login_user에 담기
  ...
  //로그인 하지 않은 사용자라면 댓글 수정삭제 불가 메시지 띄우기
  if (!login_user) {
    alert("로그인 후 댓글 수정 및 삭제가 가능합니다.");
    return;
  }
